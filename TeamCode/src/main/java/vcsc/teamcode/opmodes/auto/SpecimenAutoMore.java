...
import vcsc.teamcode.behavior.specimen.B_IntakeSpecimenAuto;
import vcsc.teamcode.behavior.specimen.B_ReleaseSpecimenAndIntakeSpecimenAuto;
...
@Disabled
@Autonomous(name = "5 + 1 Specimen Auto", group = "Auto", preselectTeleOp = "Tele")
public class SpecimenAutoMore extends OpMode {
...
//        auto.loop();
    ...
    public TaskSequence scoreSpecimen(Pose startPose) {
        DelayTask delay = new DelayTask(300);
        ...
        return new TaskSequence()
                ...
                .thenWaitUntil(() -> (delay.isFinished() && follower.getPose().getX() > SCORE_X - 2.25) || followPathTask.isFinished())
                .thenAsync(new B_ReleaseSpecimenAndIntakeSpecimenAuto()).thenDelay(100);
    }
    ...
    public void init() {
        ...
        auto.then(scoreSpecimen(startPose))
                ...
                .thenRunnable(() -> {
                    follower.startTeleopDrive();
                    follower.setTeleOpMovementVectors(-0.8, 0, 0);
                })
                .thenDelay(250)
                .then(new B_GrabSpecimenAndStowAuto())
                .then(specimenLoop(4))
